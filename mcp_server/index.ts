...
<html lang="en">
  ...
  <body>
    ...
    <ul>
      ...
      // --- Root JSON-RPC discovery compatibility (POST /)
      // Some validators post initialize/tools/list to the server root when scanning
      // MCP endpoints from a base URL. Return a finite JSON-RPC response here so
      // they don't fall through to generic SPA/static handling.
      if (req.method === 'POST' && url.pathname === '/') {
        ...
        try {
          ...
          if (methodName === 'tools/list') {
            ...
            res.end(JSON.stringify({ jsonrpc: '2.0', id: parsed?.id ?? 1, result: { tools: visibleTools } }));
            return;
          }
          ...
          res.end(
            JSON.stringify({
              jsonrpc: '2.0',
              id: parsed?.id ?? null,
              error: { code: -32601, message: methodName ? `Method not found: ${methodName}` : 'invalid_jsonrpc_request' },
            })
          );
          return;
        } catch (error: any) {
