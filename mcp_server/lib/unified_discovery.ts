  ...
  if (provider === "skiclubpro" && programRef === "309") {
    ...
    if (await childSelect.count()) {
      ...
      if (await nextBtn.count()) {
        console.log("[Test] Clicking Next to reach program options...");
        await nextBtn.click();
    ...
    // Ensure options step is visible
    await page.waitForSelector('text=Nordic Kids', { timeout: 12000 }).catch(() => {});
    await humanPause(400, 600); // Let DOM settle

    // Extract fields using standard extractor
    let schema = await extractSingleStep(page, "start");
    let fields = Object.keys(schema.properties).map((id) => ({
      id,
    ...
    // FALLBACK: If we got hidden participant fields, scrape visible-only
    const looksWrong = !fields || fields.length < 3 || fields.every(f => /participant/i.test(f.label || f.id));
    if (looksWrong) {
      console.log("[Test] Generic extractor returned hidden fields - using visible-only scraper...");
      fields = await page.evaluate(() => {
        function isVisible(el: Element) {
          const style = window.getComputedStyle(el as HTMLElement);
          const rects = (el as HTMLElement).getClientRects();
          return style && style.display !== "none" && style.visibility !== "hidden" && rects.length > 0;
        }
        function labelFor(el: HTMLElement) {
          ...
          const candidate = el.closest(".form-item, .form-group")?.querySelector("label");
        ...
        document.querySelectorAll("select").forEach((sel) => {
          ...
          const opts = Array.from(el.options).map(o => ({ label: (o.textContent||"").trim(), value: o.value }));
          out.push({
            id: el.id || el.name || `select_${Math.random().toString(36).slice(2)}`,
            label: labelFor(el),
            type: "select",
        ...
        // CHECKBOX fields (grouped by name)
        const checkBoxes = Array.from(document.querySelectorAll('input[type="checkbox"]'))
          .filter(el => isVisible(el as HTMLElement)) as HTMLInputElement[];
        const byName = new Map<string, {id:string,label:string,type:string,required:boolean,options:{label:string,value:string}[]}>();
        checkBoxes.forEach(el => {
          const name = el.name || el.id || "checkboxes";
          if (!byName.has(name)) {
            const sectionHeader = Array.from(document.querySelectorAll("h2,h3,legend,strong"))
              .find(h => /volunteer/i.test(h.textContent || ""));
            byName.set(name, {
              id: name,
              label: sectionHeader ? (sectionHeader.textContent || "").trim() : "Options",
              type: "checkbox",
              required: !!el.required || el.getAttribute("aria-required")==="true",
              options: []
            });
          ...
          byName.get(name)!.options.push({ label, value: el.value || label });
        });
        Array.from(byName.values()).forEach(v => out.push(v));

        return out;
      }) as DiscoveredField[];
    }

    console.log(`[Test] Extracted ${fields.length} visible fields from program ${programRef}`);
