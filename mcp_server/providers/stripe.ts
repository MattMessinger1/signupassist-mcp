 ...
 * Validated by @mattmessinger/refund-guard (90-day window, amount cap, double-refund protection).
 */
...
async function createCheckoutSession(args: {
  ...
  try {
    ...
    const secureLinkText = `Step 4/5 — Add a payment method securely via Stripe:\n${data.url}\n\nWe don't see or store your card details. Stripe hosts the form.`;
    return {
      success: true,
      data: {
        url: data.url,
      ...
      structuredContent: {
        ...
        step: { current: 4, total: 5, label: 'Payment (Stripe)' },
        url: data.url,
        session_id: data.session_id,
        customer_id: data.customer_id
      },
